...
export class Migration20260224083455 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "affiliate" ("id" text not null, "customer_id" text not null, "email" text not null, "code" text not null, "promotion_id" text null, "status" text check ("status" in ('pending', 'approved', 'rejected')) not null default 'pending', "commission_rate" integer not null default 10, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "affiliate_pkey" primary key ("id"));`);
